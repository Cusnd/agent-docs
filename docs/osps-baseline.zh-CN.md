...
- **证据日期：** 2026-08-18
- **当前阶段：** 发布前实现与远端控制均已验证；发布资产证据由最终发布报告补齐

Level 1 适用于代码和非代码项目。下表映射指定 Baseline 的全部 Level 1 控制项。`已满足` 表示项目证据，不代表独立认证。即使某项控制已经由已验证的发布候选物和强制 workflow 满足，发布后的远端资产回读仍是 v0.2.0 的独立验收条件。
...
- GitHub Community Profile API 返回 100%。启用规则后的 [Scorecard run](https://github.com/Cusnd/agent-docs/actions/runs/32163049236)为 commit `e600424b616294ea9d3df761c3800c1db9633d17` 发布真实分数 6.8；项目不声明分数门槛。
...
| OSPS-BR-03.02 | 已满足         | [发布说明](../RELEASING.zh-CN.md)把 HTTPS GitHub Releases 定义为唯一分发渠道，不发布 npm 或镜像；[Release workflow](../.github/workflows/release.yml)强制该渠道。                                                                                            |
...
| OSPS-LE-03.02 | 已满足         | Release verifier 强制 34 文件 ZIP allowlist 包含 `LICENSE`；[发布前 CI run](https://github.com/Cusnd/agent-docs/actions/runs/32162080649)的三个打包 job 全部通过。最终远端资产回读仍属于 Release 验收。                                                      |
...
Level 1 实现控制中不再有 `Pending`，发布候选物映射完整。badge PR 必须通过受保护检查后才能创建 `v0.2.0` tag。只有最终发布报告记录不可变 tag、远端 ZIP 与 checksum、attestation、发布后外链结果和远端下载物隔离安装，才可以宣称发布完成。
